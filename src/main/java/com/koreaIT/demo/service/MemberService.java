...
public class MemberService {
	...
	public ResultData<Integer> doJoin(String loginId, String loginPw, String name, String nickname, String cellphoneNum,
		...
		if (existsMember != null) {
			return ResultData.from("F-10", Util.f("이미 사용중인 이름(%s)과 이메일(%s) 입니다", name, email));
		}

		memberRepository.doJoin(loginId, loginPw, name, nickname, cellphoneNum, email);

		return ResultData.from("S-1", Util.f("%s회원님이 가입되었습니다", loginId),"member", memberRepository.getLastInsertId());
	}
	...
	public boolean MemberLoginChk(HttpSession httpsession) {
